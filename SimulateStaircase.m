function [curve_cont,trls_cont,curve_blkd,trls_blkd,curve_noncons,trls_noncons,curve_wtd,trls_wtd] = SimulateStaircase(s0,ds,ntrls,M,N,error)

[curve_cont,trls_cont] = ContStairSim(s0,ds,ntrls,M,N,error);
[curve_blkd,trls_blkd] = BlockStairSim(s0,ds,ntrls,M,N,error);
[curve_noncons,trls_noncons] = NonConsecStairSim(s0,ds,ntrls,M,N,error);
gamma = N; [curve_wtd,trls_wtd] = WeightedStairSim(s0,ds,ntrls,gamma,error);
...
function [s,trls] = ContStairSim(s0,ds,ntrls,M,N,error)
s = zeros(ntrls,1);
s(1) = s0;
for i = 1:ntrls
...
function [s,trls] = BlockStairSim(s0,ds,ntrls,M,N,error)
s = zeros(ntrls,1);
s(1) = s0;
for i = 1:ntrls
...
function [s,trls] = NonConsecStairSim(s0,ds,ntrls,M,N,error)
s = zeros(ntrls,1);
s(1) = s0;
for i = 1:ntrls
    p = erf(s(i)/sqrt(2))/2 + 0.5; % added missing 0.5, possible bug int he previous version??
...
function [curve,trls] = WeightedStairSim(s0,ds,ntrls,gamma,error)
s = zeros(ntrls,1);
s(1) = s0;
